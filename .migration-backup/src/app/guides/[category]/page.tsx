...
import { getGuidesByCategory, getCategoryBySlug } from "@/lib/guides";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${cat.name} Guides — ${SITE_NAME}`,
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-12">
      <header className="mb-10">
        <Link href="/guides" className="text-sm text-blue-600 hover:underline mb-2 inline-block">
          ← All Guides
        </Link>
      ...
      {guides.length === 0 ? (
        <div className="text-center py-20 text-gray-400">
          ...
          <p className="mt-2">
            <Link href="/guides" className="text-blue-600 underline">
              Browse all guides
            </Link>
          </p>
        </div>
